...
5. Are there specific rules, standards, or a checklist this change must satisfy — a compliance rule, security policy, industry standard, legal requirement? If yes, either point me to a file with `rubric:<path>`, or just list the rules here and I will write the checklist file for you. If you don't know what rules should apply, say so — I can draft a checklist from what I know about this change and the codebase, and you can correct it.
```

Use the user's answers to build a focused test expectation summary. If the user answered question 5 with rules but no file, record those rules in conversation state as `INLINE_RUBRIC_ITEMS` (one checklist item per line) — the user should not have to pre-author a rubric file to use one.

If the user answers question 5 with "I don't know," "you decide," or similar — not naming the rules but not declining either — the checklist obligation does not fall back to the user by default. The builder must draft one: use its own knowledge of the change (the category detected in floor-detection, the framework/language conventions visible in the diff, and common failure modes for that category — e.g. for billing: idempotency on retry, no double-charge, currency/rounding precision, refund path correctness; for auth: session invalidation on logout, permission checks at every entry point, no privilege escalation via parameter tampering; for migrations: rollback path, backward compatibility with in-flight data, no silent data loss) to write a draft `INLINE_RUBRIC_ITEMS` checklist, then show it to the user as a draft and ask them to confirm, edit, or drop items before it is used — never send a builder-drafted rubric to the reviewer without the user seeing it first, since an unconfirmed checklist could assert rules the user never actually holds the code to.

`TMP_ROOT` and `REVIEW_ID` do not exist yet at this step (they are created in Step 5); do not write any file here. Present the summary (and the rubric items, whether user-supplied or builder-drafted) back to the user for approval before including them in the reviewer prompt. If `INLINE_RUBRIC_ITEMS` is non-empty, treat it as equivalent to `RUBRIC_PATHS` being non-empty for the rest of this audit (floor-detection nudges, Step 7 rubric injection, Step 11/12 rubric-coverage checks); in Step 7, write `INLINE_RUBRIC_ITEMS` to `${TMP_ROOT}/advreview-rubric-${REVIEW_ID}.md` before building the rubric block, now that those variables exist. If the user explicitly declines any checklist (not just "I don't know" but "skip it" / "no rubric"), proceed with the audit but note in the reviewer prompt that no focused test expectations or rubric were available.
...
- rubric: if there are specific rules this must satisfy (a compliance rule, security policy, standard), tell me and I will check the code against them item by item — or point me to a file with rubric:<path>. Not sure what rules should apply? Say so and I will draft a checklist from what I know about this change, and you can correct it.
...
Neither is required to continue. Reply with rubric items, a rubric:<path>, "draft one for me," or "skip" to continue without one.
```

Wait for the user's reply before proceeding to Step 5. On "skip," continue with `RUBRIC_PATHS` empty exactly as before — this nudge fires once per audit and never blocks. If the user provides rubric items inline, fold them into `INLINE_RUBRIC_ITEMS` alongside anything captured in the test-spec question above. If the user asks the builder to draft one ("draft one for me," "you decide," "I don't know"), write a draft checklist using the matched `FLOOR_CATEGORIES` and the specifics of this diff — e.g. a `money/billing` match should draft items like idempotency on retry and no double-charge; `auth/permissions` should draft items like permission checks at every entry point and no privilege escalation via parameter tampering; `migrations/destructive-data` should draft items like a rollback path and no silent data loss — then show the draft to the user for confirmation or edits before using it. Never pass a builder-drafted checklist to the reviewer without the user seeing it first.
